...
지상 디펜스는 성채 앞 방어선이 적 웨이브를 버티는 자동 진행 모드다. 성채는 절대 안전하며, 적은 성채를 함락시키지 못한다.
...
지상전의 버튼 단위 플로우와 적 행동 규칙은 `GroundBattleFlowBlueprint.md`를 기준으로 한다. 상위 기획서와 세부 규칙이 충돌할 경우, 지상 디펜스 구현은 별도 청사진 문서를 우선한다.
...
웨이브는 지상 디펜스의 난이도와 보상을 올리는 단위다. 플레이어는 적진을 공격하지 않고, 성채 앞 방어선을 강화해 더 오래 버틴다.
...
웨이브를 막으면 다음 웨이브로 올라가고 보상이 증가한다. 막지 못하면 웨이브 단계는 오르지 않고, 이번 웨이브 보상도 얻지 못한다. 성채는 안전하므로 게임오버는 없지만, 방어를 강화하지 않으면 성장이 느려진다.
