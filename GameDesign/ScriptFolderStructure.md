...
| UI | `Assets/02.Scripts/UI/PlayableLoopHud.cs`, `PlayableScreenLayoutController.cs`, `PanelCameraRenderTarget.cs`, `DungeonViewportInputRouter.cs` | Normal player HUD, first-session guidance/recovery copy including no-save `Load` guidance, HUD settings snapshot/apply, focus/overlays, viewport render bridge, dungeon viewport input. |
