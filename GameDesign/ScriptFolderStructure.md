...
- Player-facing UI shows actions, consequences, first-session next steps, recovery meaning, compact reward comparison, compact defense-upgrade comparison, and missing Wall resources when wall/pressure is the current blocker; unresolved or unequipped latest rewards should keep their `Next:` decision before upgrade or next-contract guidance. Do not show review/debug/render wiring status.
