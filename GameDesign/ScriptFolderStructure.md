...
Current implementation note: `GroundDefenseBalanceModel`, `DefenseDirector`, and `DefenseRuntimeState` own the formula-driven frontline simulation. `GroundDefenseEnemyArchetype`, `GroundDefenseActorRuntime`, and `GroundDefenseEnemyPool` provide reusable role data, transient actor state, and pooling. `GroundDefenseBillboardUtility`, `GroundDefenseEnemyView`, `GroundDefenseBattlefieldView`, and the pulse/bolt portions of `GroundDefenseCombatPresenter` describe the current rejected presentation bridge, not the approved final direction. E0-A is `Next` again: preserve the simulation/pooling foundations, but replace normal-path billboard props and pulse-derived attacks with RTS-readable automatic squad combat, real attacker-owned projectiles/contact, deaths, reinforcements, and wall-bound damage. Individual unit controls, production queues, free tower placement, manual waves, and a second save/progression model remain out of scope.

Automation tool note: `Tools/Automation/Invoke-IncrementalDiabloChecks.ps1` now requires the canonical plan to keep E0-A as `RTS-readable automatic defense battlefield | Next` and to preserve the real-projectile/reinforcement direction. It still guards the reusable pooled archetype/runtime scene foundation, but those checks do not mark the rejected billboard/pulse presentation as accepted.
