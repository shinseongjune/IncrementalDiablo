...
| UI | `Assets/02.Scripts/UI/PlayableLoopHud.cs`, `PlayableScreenLayoutController.cs`, `PanelCameraRenderTarget.cs`, `DungeonViewportInputRouter.cs` | Normal player HUD, accepted first-session guidance/recovery copy including no-save `Load` guidance, HUD settings snapshot/apply, E2-B contract `Goal:` text plus latest-item equip/salvage comparison text, focus/overlays, viewport render bridge, and dungeon viewport input. |
...
- Player-facing UI shows actions, consequences, first-session next steps, recovery meaning, and compact reward comparison, not review/debug/render wiring status.
