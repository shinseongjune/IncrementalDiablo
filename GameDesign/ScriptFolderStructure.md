...
Current implementation note: `GroundDefenseBalanceModel`, `DefenseDirector`, and `DefenseRuntimeState` own the formula-driven frontline simulation. `GroundDefenseEnemyArchetype`, `GroundDefenseActorRuntime`, and `GroundDefenseEnemyPool` provide reusable role data, transient actor state, lifecycle events, and pooling. `GroundDefenseBattlefieldView` owns an explicit E0-A1 `StaticGrammar` stage that creates staging/approach/contact/protected regions while hiding pooled motion and combat events. User validation confirmed the regions but rejected the actor/building read: the actual panel still shows primitive rectangles/capsules/blocks instead of recognizable enemy, defender, tower, and wall silhouettes. E0-A is `Next / Validation failed`; investigate the generated billboard material/UV/alpha/sorting/camera and primitive fallback path before enabling `AutomaticBattle`. Individual unit controls, production queues, free tower placement, manual waves, and a second save/progression model remain out of scope.

Automation tool note: `Tools/Automation/Invoke-IncrementalDiabloChecks.ps1` requires the canonical plan to keep E0-A as `RTS-readable automatic defense battlefield | Next / Validation failed`, requires `presentationStage: 0` in `Gameplay`, and guards the static zone/proof tokens before the later `Unit -> action -> target` and attacker-owned projectile gates.
