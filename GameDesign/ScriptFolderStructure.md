...
Current implementation note: `GroundDefenseBalanceModel`, `DefenseDirector`, and `DefenseRuntimeState` own the formula-driven frontline simulation. `GroundDefenseEnemyArchetype`, `GroundDefenseActorRuntime`, and `GroundDefenseEnemyPool` provide reusable role data, transient actor state, lifecycle events, and pooling. `GroundDefenseBattlefieldView` and `GroundDefenseCombatPresenter` are currently a rejected presentation checkpoint: the user saw rapid top-to-bottom enemy motion and an ownerless projectile near the wall rather than distinct RTS units, buildings, and attacks. E0-A is `Next / Validation failed`. The next code pass must implement E0-A1 static RTS nouns with one enemy, one defender, one tower, one wall, and stable battlefield zones before restoring density. Individual unit controls, production queues, free tower placement, manual waves, and a second save/progression model remain out of scope.

Automation tool note: `Tools/Automation/Invoke-IncrementalDiabloChecks.ps1` requires the canonical plan to keep E0-A as `RTS-readable automatic defense battlefield | Next / Validation failed` and preserves the new implementation-order tokens: `Static RTS battlefield grammar`, `Unit -> action -> target`, and `attacker-owned projectile`.
