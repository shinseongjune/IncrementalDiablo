...
**Last meaningful movement:** E2-B contract comparison clarity now has a structural first pass. `DungeonContractModel.FormatGoalComparisonText(...)` names the selected contract's goal against the other offered contract, and `PlayableLoopHud` shows that `Goal:` line plus a compact `Next:` action hint before dungeon entry. E2-A remains accepted from the 2026-07-01 user-confirmed recovery guidance check.

**Next unlock:** `E2-B | P1 | Goal comparison clarity | In progress / Contract comparison core implemented`. Focused Play Mode validation should confirm the compact Dungeon line and `Next:` hint let a player distinguish safer clear/recovery from higher reward-depth contracts without crowding the HUD.
...
| E2-B | P1 | Goal comparison clarity | In progress / Contract comparison core implemented | Help a fresh player name the better next choice after recovery works. | `DungeonContractModel` now formats selected-vs-alternative contract goals from reward-depth and listed threat, and `PlayableLoopHud` shows that guidance in the compact contract block, contract selection message, refresh message, and first-session `Next:` hint. Remaining evidence: focused `Gameplay` Play Mode validation for A/B/refresh -> compare `Goal:` copy -> start selected run -> confirm the guidance stays concise. |
...
| Dungeon reward -> inventory/equipment/salvage | Working and accepted with authored affix output, encounter reward-depth offsets, and E2-A recovery guidance | E2-B contract comparison is implemented structurally; later E2-B passes can add latest-item or upgrade comparison after the contract text is Play Mode accepted. |
