...
**Last meaningful movement:** E2-A no-save recovery guidance now has a single runtime owner in `DefenseSaveManager.NoSaveRecoveryGuidance`. `PlayableLoopHud` reuses that copy, and the harness checks both the save manager and normal HUD so a first-session `Load` attempt cannot regress to a raw save-file path before Play Mode QA.

**Next unlock:** `E2-A | P1 | Onboarding, settings, recovery | In progress / No-save recovery guidance hardened`. Remaining unlock evidence is focused fresh-save/recovery Play Mode QA, plus a product decision before adding a full player-facing settings menu beyond the current HUD text-density/guide toggles.
...
| E2-A | P1 | Onboarding, settings, recovery | In progress / No-save recovery guidance hardened | Teach the first-session loop after E1-A makes a real decision. | Normal HUD now uses compact status plus `Next:` guidance for start frontline -> compare contracts -> run/fail/reward -> equip/salvage -> first recovery save. Save schema v6 persists HUD text density, balance-detail visibility, diagnostic text visibility, first-session guide, and first-recovery-save emphasis. The `Load` button remains clickable before the first save, and `DefenseSaveManager`/`PlayableLoopHud` share the same no-save recovery copy under harness coverage. Focused fresh-save/recovery Play Mode QA remains. |
...
| Save/load -> progression recovery | Working for depth/items/defense/contracts/affixes/encounters; first-save HUD copy explains the recovery point; schema v6 persists the current HUD text-density/guide settings; no-save load guidance is shared by `DefenseSaveManager` and the HUD | E2-A still needs focused recovery QA from a fresh save. |

Known blockers: no source-code blocker. E2-A still needs focused Play Mode QA from a true fresh-save path, including clicking `Load` before the first save. A broader player-facing settings menu needs a product decision on which settings are in scope; the current implementation only persists the existing HUD text-density/guide settings. The optional automation prompt update still needs user approval before the TOML freshness warnings can disappear.
