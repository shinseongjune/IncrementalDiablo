...
| Last meaningful movement | 2026-05-31: dungeon clear now opens the wired reward overlay from the normal `PlayableLoopHud` path when `openRewardOverlayOnDungeonClear` is enabled. `PlayableScreenLayoutController.TryOpenOverlayAfterGameplayFocus(...)` applies the return gameplay focus first, so the reward overlay can close back to `DefenseFocus` without leaving a half-transitioned dungeon/defense split. |
| Next unlock | Play Mode validate the automatic reward-overlay path: start/clear the authored dungeon room, confirm `RewardOverlay` opens automatically, confirm loot source/reward details show the authored table reward, equip or salvage the reward from the overlay, open inventory from the reward overlay, close overlays, and confirm the screen returns to `DefenseFocus`. Then continue the crafting path: with no Rare in inventory, clear or claim the next authored dungeon reward and confirm it is Rare, salvage one Rare if `AlterStone` is still missing, claim the next Rare, open crafting from the bottom action bar, select the Rare item, reroll the Rare affix, confirm materials are spent, confirm the affix text changes, confirm equipped hero stats refresh if the item is equipped, salvage a spare item, close overlays, and confirm the previous focus returns correctly. Ground-combat feel validation remains a parallel scene/feel gate. |
| Loop coverage | Phase A debug loop and Phase B player HUD slice are confirmed. Phase C now has one direct-control dungeon encounter path, visible room-presentation support, `Gameplay` wiring for `PF_DungeonEnemy_Melee` through `EnemySpawner`, authored tier-1 item definitions in the reward loop with first-Rare/pity access, HUD diagnostics that expose prototype fallback, a verified ground-lane presentation bridge, a `Gameplay`-wired ground combat feedback presenter with runtime combat telemetry, a reusable screen-focus/overlay controller for the first dual-panel playable screen, HUD-facing overlay controls with missing-overlay guards, an inventory overlay presenter for item review/equip/salvage, a reward overlay presenter that can auto-open on dungeon clear and return to the intended gameplay focus, a crafting overlay presenter plus first-pass scene layout for Rare affix reroll/salvage, and a scriptable automation check for the current scene/docs/config contract. Play Mode validation for real ground-defense feel, the authored dungeon room shell, layout transition readability, reward auto-open/readability, crafting overlay interaction/readability, and long-term item registry/drop-table tooling remain open. |
| Known blockers | There is no current code build blocker. Product blockers remain: `GroundDefenseCombatPresenter` needs Play Mode feel validation; direct-control dungeon encounter and authored reward path still need full Play Mode feel/loop validation; room scale/layout, spawn placement, NavMesh feel, marker art/count, lane/camera framing, panel crop, and final visual composition still require manual Unity judgment. Static scene checks now find `Panel_InventoryOverlay`, `Panel_CraftingOverlay`, and `Panel_RewardOverlay` wired into `PlayableScreenLayoutController`, and the reward overlay can now be invoked by the normal room-clear path. Final split ratio, camera feel, overlay content density, reward reveal treatment, crafting panel density, and ornate UI density still require user/Unity Editor review. |
