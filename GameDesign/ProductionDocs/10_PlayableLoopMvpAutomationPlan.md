...
| Last meaningful movement | 2026-05-27: `PlayableScreenLayoutController` now refuses to enter an overlay focus when the matching overlay GameObject is not wired, and `PlayableLoopHud` exposes button-safe inventory/crafting/reward overlay entry plus close controls and screen-focus status text. This moves the screen bridge from hidden controller-only behavior toward a normal player HUD path. |
| Next unlock | In Unity, wire `Panel_InventoryOverlay`, `Panel_CraftingOverlay`, and `Panel_RewardOverlay` into `PlayableScreenLayoutController`, then add bottom-action-bar buttons to `PlayableLoopHud` for `OpenInventoryOverlay`, `OpenCraftingOverlay`, `OpenRewardOverlay`, and `CloseOverlay` if the layout has room. Play Mode validate: start in `DefenseFocus`, press `Start Dungeon`, confirm dungeon becomes dominant while defense compresses to the right-side panel, clear/fail back to `DefenseFocus`, then open/close each wired overlay and confirm it returns to the previous gameplay focus. Ground-combat feel validation remains a parallel scene/feel gate. |
| Loop coverage | Phase A debug loop and Phase B player HUD slice are confirmed. Phase C now has one direct-control dungeon encounter path, visible room-presentation support, `Gameplay` wiring for `PF_DungeonEnemy_Melee` through `EnemySpawner`, authored tier-1 item definitions in the reward loop, HUD diagnostics that expose prototype fallback, a verified ground-lane presentation bridge, a `Gameplay`-wired ground combat feedback presenter with runtime combat telemetry, a reusable screen-focus/overlay controller for the first dual-panel playable screen, HUD-facing overlay controls with missing-overlay guards, and a scriptable automation check for the current scene/docs/config contract. Play Mode validation for real ground-defense feel, the authored dungeon room shell, layout transition readability, player-facing inventory content, overlay visual composition, and long-term item registry/drop-table tooling remain open. |
| Known blockers | There is no current code build blocker. Product blockers remain: `GroundDefenseCombatPresenter` needs Play Mode feel validation; direct-control dungeon encounter and authored reward path still need full Play Mode feel/loop validation; room scale/layout, spawn placement, NavMesh feel, marker art/count, lane/camera framing, panel RectTransform composition, and final visual composition still require manual Unity judgment. Optional inventory/crafting/reward overlay GameObjects are still unwired and the harness reports them as warnings until authored; overlay buttons should stay disabled until those references exist. Final split ratio, camera feel, panel crop, overlay content density, and ornate UI density still require user/Unity Editor review. |
...
Status: Code bridge plus HUD overlay access added (Unity panel wiring and Play Mode validation pending)
...
- The current `Gameplay` scene still needs authored overlay panels and button hookup before the normal player can use this visually. Until then, the overlay buttons should be disabled and the harness warning is expected.
