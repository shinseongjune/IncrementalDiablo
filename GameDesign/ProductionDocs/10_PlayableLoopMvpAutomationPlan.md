...
| Last meaningful movement | 2026-05-29: `RewardOverlayPresenter` now gives a wired `Panel_RewardOverlay` player-facing reward reveal path, and `Gameplay` has a first deterministic reward-overlay layout pass using the centered overlay frame, left reward summary, middle item/material preview, and right action stack. This moves dungeon clear rewards closer to a normal UI reveal instead of only HUD/latest-item text. |
| Next unlock | Play Mode validate the reward-overlay path now that the objects are created, connected, and placed: clear a dungeon, open reward overlay, confirm loot source/reward details, equip or salvage the reward, open inventory from the reward overlay, close overlays, and confirm the previous `DefenseFocus`/`DungeonFocus` returns. If inventory overlay presenter wiring is incomplete in the local scene, finish that wiring before judging the reward-to-inventory handoff. `Panel_CraftingOverlay` is already present as an overlay GameObject but still needs content behavior later. Ground-combat feel validation remains a parallel scene/feel gate. |
| Loop coverage | Phase A debug loop and Phase B player HUD slice are confirmed. Phase C now has one direct-control dungeon encounter path, visible room-presentation support, `Gameplay` wiring for `PF_DungeonEnemy_Melee` through `EnemySpawner`, authored tier-1 item definitions in the reward loop, HUD diagnostics that expose prototype fallback, a verified ground-lane presentation bridge, a `Gameplay`-wired ground combat feedback presenter with runtime combat telemetry, a reusable screen-focus/overlay controller for the first dual-panel playable screen, HUD-facing overlay controls with missing-overlay guards, an inventory overlay presenter for item review/equip/salvage, a reward overlay presenter with first-pass UI placement for reward reveal/equip/salvage/open-inventory actions, and a scriptable automation check for the current scene/docs/config contract. Play Mode validation for real ground-defense feel, the authored dungeon room shell, layout transition readability, inventory/reward overlay composition, crafting overlay content, and long-term item registry/drop-table tooling remain open. |
| Known blockers | There is no current code build blocker. Product blockers remain: `GroundDefenseCombatPresenter` needs Play Mode feel validation; direct-control dungeon encounter and authored reward path still need full Play Mode feel/loop validation; room scale/layout, spawn placement, NavMesh feel, marker art/count, lane/camera framing, panel crop, and final visual composition still require manual Unity judgment. Static scene checks now find `Panel_InventoryOverlay`, `Panel_CraftingOverlay`, and `Panel_RewardOverlay` wired into `PlayableScreenLayoutController`; the remaining overlay gap is Play Mode readability/interaction validation for inventory/reward and a future crafting presenter. Final split ratio, camera feel, overlay content density, reward reveal treatment, and ornate UI density still require user/Unity Editor review. |
