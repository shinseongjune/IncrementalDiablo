...
**Last meaningful movement:** E2-B contract comparison clarity was user-accepted on 2026-07-03, so it is regression-only. The next E2-B comparison slice now has a structural first pass plus a guide-off priority hardening pass: `PlayableLoopHud` compares the latest reward item against the currently equipped same-slot item, turns the result into compact Item text, and keeps the latest item equip/salvage `Next:` hint ahead of the next-contract hint even when the first-session guide is disabled. E2-A remains accepted from the 2026-07-01 user-confirmed recovery guidance check.

**Next unlock:** `E2-B | P1 | Goal comparison clarity | In progress / Latest item comparison hint priority implemented`. Focused Play Mode validation should confirm a normal reward makes the compact Item line and `Next:` hint distinguish equip upgrade, empty-slot equip, sidegrade, or salvage-spare decisions without crowding the HUD, including the guide-off setting path.
...
| E2-B | P1 | Goal comparison clarity | In progress / Latest item comparison hint priority implemented | Help a fresh player name the better next choice after recovery works. | Contract comparison is accepted from the 2026-07-03 user-confirmed check: A/B/refresh changed `Goal:` copy and the compact `Next:` hint stayed readable. `PlayableLoopHud` now compares the latest reward item against the currently equipped same-slot item using existing resolved item level/power state, and `TryBuildLatestItemDecisionHint(...)` keeps unresolved or unequipped latest items ahead of the ready-state contract hint even when the first-session guide is disabled. It changes no drop odds, reward denominator, salvage yield, save schema, or scene layout. Remaining evidence: focused `Gameplay` Play Mode validation for reward -> Item `Compare:` text -> `Next:` equip/salvage hint -> equip or salvage -> save/load, with guide-on default and guide-off priority checked if practical. |
