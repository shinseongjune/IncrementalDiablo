...
| Last meaningful movement | 2026-06-20: E0-B now has a conservative, anchor-derived `Camera_DefensePanel` starting preset: its vertical screen axis follows the existing enemy-spawn-to-wall travel axis, placing enemy pressure on the top/far side and the protected wall at the lower side without changing anchors or combat logic. |
| Next unlock | Run the focused E0-B Play Mode comparison at real Level 1 and the Level 21 review band in `DefenseFocus` and the compressed panel. Accept only if the new top-to-bottom pressure hierarchy preserves the accepted E0-A actor identity, attacker ownership, target reaction, death/reinforcement, and wall damage. |
...
| Known blockers | No runtime-combat, save, item, or economy blocker is known. E0-B is a visual judgment gate: the anchor-derived camera preset and render bridge are ready, but a focused Unity Editor/Play Mode comparison must decide whether the top-to-bottom reference read is obvious at both panel scales. |
...
- 2026-06-19: Added a non-save E0-A3 review override to remove the remaining validation setup friction. `GroundDefenseNavMeshBattlefield` can preview Level 1, Level 11, or Level 21 visual force profiles through context menu methods without changing `DefenseRuntimeState`, save data, rewards, or offline progression. `PlayableLoopHud` auto-finds the battlefield and shows `E0-A3 review scale` only while the override is active, so Play Mode review can distinguish real progression from review-only density. E0-A3 remains `Needs Unity Play Mode`; this run added the missing validation path, not acceptance evidence.
- 2026-06-19: User confirmed the E0-A3 high-band density/role-mix view reads well enough in Play Mode. E0-A is now accepted through E0-A3. The user also noted that matching the reference image likely requires a camera-position/framing change; track that as E0-B camera/composition work, not as an E0-A readability rejection.
- 2026-06-19: Advanced E0-B only on the deterministic setup side. `PanelCameraRenderTarget` now has `Render Target/Apply Now` for Inspector-side rebinding, and the automation harness separately guards `RawImage_DungeonViewport` and `RawImage_DefenseViewport` so the compressed-defense camera view cannot silently lose its render bridge before the manual composition pass. This does not choose camera values; E0-B still needs Unity Editor judgment.
- 2026-06-19: User review after the E0-B setup pass: the view is readable, but the change is not visually obvious and the target reference reads as enemies massing from the top of the screen toward the lower defense. The preferred next E0-B implementation attempt is camera/composition first: make `Camera_DefensePanel` and the defense viewport crop present the accepted NavMesh battlefield as top/far-side enemy pressure moving toward a lower protected wall. Do not change combat logic, add manual waves, or reopen E0-A readability unless camera-only composition cannot preserve the accepted ownership read.

- 2026-06-20: Applied the first reversible E0-B camera composition preset without moving `EnemySpawnAnchor`, `WallAnchor`, NavMesh, units, formulas, or UI anchors. `Camera_DefensePanel` now faces along the existing world `+X -> -X` pressure route with its screen vertical axis running from the enemy spawn at the top/far side toward the protected wall at the lower side; orthographic size expands from `5` to `10.25` so the 19.1-unit generated lane fits vertically. This is a Play Mode comparison candidate, not E0-B acceptance evidence.
...
| ID | Priority | Track | Current status | Completion criteria | Next update required |
| --- | --- | --- | --- | --- | --- |
| E0-A | P0 | RTS-readable automatic defense battlefield | Done / E0-A3 accepted | One actual NavMesh battlefield uses real autonomous combatants. E0-A1 is accepted: enemies and defenders move, engage, die/reinforce, surviving enemies attack the wall, and wall health changes through the authoritative runtime. E0-A2 is accepted: cutout friendly/enemy actors, generated faction markers, attacker-owned melee/wall-hit lines, and target recoil read well enough in Play Mode. E0-A3 is accepted: force count, deterministic enemy role mix, and reinforcement cadence follow the Frontline formula profile without unit micromanagement or manual wave rows, and the high-band density remains readable in Play Mode. The non-save review override stays as verification support only. | No further E0-A readability work unless a regression appears. Clear the review override before judging normal progression. |
| E0-B | P0 | Defense camera and reference composition pass | In Progress / Anchor-derived starting preset applied / Needs Unity Editor composition judgment / Static bridge guarded | Preserve the accepted E0-A actor identity, attacker ownership, role mix, density, and automatic behavior while moving the defense camera/framing toward `GameDesign/References/2026-05-17_FinalGameplayScreenConcept.png`, especially the read of enemies pressing from the top/far side toward the lower protected wall. The current conservative preset looks from `(-3.56, 11.21, 69.31)` at yaw `90` with orthographic size `10.25`, so the unchanged `EnemySpawnAnchor` at world `+X` maps to screen top and `WallAnchor` at world `-X` maps to screen bottom. Do not add unit controls, manual waves, or a new presentation abstraction. | Focused manual pass: use the applied preset first, then validate real Level 1 and E0-A3 review-band density in both `DefenseFocus` and the compressed panel; use `Render Target/Apply Now` on the relevant `PanelCameraRenderTarget` if a viewport needs rebinding. If the hierarchy is still not obvious, stop before anchor edits and propose scene-anchor rotation/repositioning options. |
...
3. `E0-A3 - Formula-driven battle scale`
   ...
   - 2026-06-19 review path: use the battlefield's non-save review Frontline Level override to preview Level 11 or Level 21 density without changing `DefenseRuntimeState`.
   - Cadence must leave enough time to recognize movement, attack, hit, death, and wall pressure.
   - Accepted in Play Mode on 2026-06-19. The remaining camera/reference-image framing note belongs to E0-B, not to E0-A3 readability.
...
- Current checkpoint: the actual NavMesh movement, melee engagement, readable attack ownership, death/reinforcement, wall-damage path, and E0-A3 formula-driven force density/roles/cadence are accepted. The next proof is a camera/reference-image composition pass that preserves this readability in both panel states.
...
- Operating rule: E0-A must proceed from real gameplay objects, not explanatory bands or diagnostic geometry. The actual ground, NavMesh movement, defender interception, enemy targeting, melee damage, death/respawn, wall attacks, readable melee/wall-hit ownership, and formula-driven role mix/density are accepted. Next preserve those reads while tuning camera/reference composition. Do not build unit selection, movement commands, focus fire, production queues, worker economy, free tower placement, or hand-authored wave lists.
