...
**Last meaningful movement:** E3-A first-session QA checklist was drafted on 2026-07-09 as a reproducible fresh-save -> no-save Load -> contract -> run -> reward -> equip/salvage -> defense-upgrade -> save/load path. E2-B goal comparison clarity remains user-confirmed from 2026-07-08 and is regression-only. E2-A remains accepted from the 2026-07-01 user-confirmed recovery guidance check.

**Next unlock:** `E3-A | P1 | Settings menu scope and first-session QA checklist | Needs product decision`. Stop adding more E2-B HUD comparison micro-slices unless a regression is reported. The R3 first-session QA checklist now exists; the next product decision is whether R3 should expose the existing persisted HUD settings in a small player-facing settings menu, turn the checklist into the build handoff, or start a manual visual-authoring pass for boss/room presentation.
...
| E3-A | P1 | Settings menu scope and first-session QA checklist | Needs product decision | Move beyond E2-B HUD comparison work toward R3 readiness. | 2026-07-09: the reproducible R3 first-session QA checklist was drafted in `06_UnitySceneAndPrefabSetupGuide.md`. Recommended next decision: expose only the existing persisted HUD text-density/guide toggles first, because the save data already exists and it avoids a broad layout/settings rewrite. Alternatives are to treat the checklist as the build handoff first, or to start a manual visual-authoring pass for boss/room presentation after the user approves visual scope. |
...
| Dungeon reward -> inventory/equipment/salvage | Working and accepted with authored affix output, encounter reward-depth offsets, E2-A recovery guidance, accepted E2-B contract comparison, user-confirmed latest-item comparison, accepted defense-upgrade comparison, and an E3-A first-session QA checklist | E2-B is closed unless reward comparison, defense-upgrade guidance, or save/load regresses. The next gap is not another comparison line; it is deciding the first R3-facing settings menu/build-handoff/presentation scope. |
...
Known blockers: no source-code blocker. E2-B is accepted and should not receive another comparison micro-slice without a regression. E3-A now has the fresh-save QA checklist, but still needs a product decision: small settings menu for existing persisted HUD settings, build handoff around the checklist, or manual visual-authoring scope for boss/room presentation. The optional automation prompt update still needs user approval before the TOML freshness warnings can disappear.
