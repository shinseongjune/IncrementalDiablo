...
**Last meaningful movement:** E2-B goal comparison clarity was user-confirmed on 2026-07-08. Contract comparison, latest-item comparison, guide-off latest-item priority, defense-upgrade comparison, Wall shortfall, and post-upgrade return guidance are accepted and regression-only. E2-A remains accepted from the 2026-07-01 user-confirmed recovery guidance check.

**Next unlock:** `E3-A | P1 | Settings menu scope and first-session QA checklist | Needs product decision`. Stop adding more E2-B HUD comparison micro-slices unless a regression is reported. The next product decision is whether R3 should first expose the existing persisted HUD settings in a small player-facing settings menu, create a reproducible fresh-save QA checklist/build handoff, or start a manual visual-authoring pass for boss/room presentation.
...
| E3-A | P1 | Settings menu scope and first-session QA checklist | Needs product decision | Move beyond E2-B HUD comparison work toward R3 readiness. | Recommended next decision: expose only the existing persisted HUD text-density/guide toggles first, because the save data already exists and it avoids a broad layout/settings rewrite. Alternatives are a reproducible fresh-save QA checklist/build handoff, or a manual visual-authoring pass for boss/room presentation after the user approves visual scope. |
...
| Dungeon reward -> inventory/equipment/salvage | Working and accepted with authored affix output, encounter reward-depth offsets, E2-A recovery guidance, accepted E2-B contract comparison, user-confirmed latest-item comparison, and accepted defense-upgrade comparison | E2-B is closed unless reward comparison, defense-upgrade guidance, or save/load regresses. The next gap is not another comparison line; it is deciding the first R3-facing settings/QA/presentation scope. |
...
Known blockers: no source-code blocker. E2-B is accepted and should not receive another comparison micro-slice without a regression. E3-A needs a product decision: small settings menu for existing persisted HUD settings, fresh-save QA/build handoff, or manual visual-authoring scope for boss/room presentation. The optional automation prompt update still needs user approval before the TOML freshness warnings can disappear.
