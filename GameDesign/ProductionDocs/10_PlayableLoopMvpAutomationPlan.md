...
**Last meaningful movement:** E2-B latest reward item comparison was user-confirmed on 2026-07-05, so the contract and latest-item comparison slices are regression-only. The current E2-B defense-upgrade slice now recommends the best currently affordable upgrade, and the 2026-07-07 pass keeps stressed wall/pressure goals on Wall by showing the exact missing Gold/Scrap when Wall is the right next upgrade but cannot yet be bought. E2-A remains accepted from the 2026-07-01 user-confirmed recovery guidance check.

**Next unlock:** `E2-B | P1 | Goal comparison clarity | In progress / Defense upgrade shortfall guidance implemented`. Focused Play Mode validation should confirm that, after reward handling is resolved, the compact `Next:` hint names the recommended Wall, Tower, or Defenders upgrade when it is affordable; when wall/pressure is the blocker and Wall is not yet affordable, it should show the missing Gold/Scrap for Wall instead of recommending a weaker goal. Buying the named upgrade should still route back to Push or the next contract without crowding the HUD.
...
| E2-B | P1 | Goal comparison clarity | In progress / Defense upgrade shortfall guidance implemented | Help a fresh player name the better next choice after recovery works. | Contract comparison is accepted from the 2026-07-03 user-confirmed check. Latest reward item comparison is accepted from the 2026-07-05 user-confirmed check, including guide-off priority for unresolved or unequipped latest rewards. `PlayableLoopHud` now adds a defense-upgrade comparison after reward handling: stressed wall/pressure favors Wall and, if Wall is not affordable, shows the exact missing Gold/Scrap; otherwise affordable Tower/Defenders are compared by existing DPS gain before next-contract guidance. It changes no drop odds, reward denominator, salvage yield, save schema, upgrade cost, or scene layout. Remaining evidence: focused `Gameplay` Play Mode validation for stressed-wall shortfall text, enough Gold/Scrap -> defense upgrade `Next:` recommendation -> buy the named upgrade -> save/load. |
...
| Dungeon reward -> inventory/equipment/salvage | Working and accepted with authored affix output, encounter reward-depth offsets, E2-A recovery guidance, accepted E2-B contract comparison, and user-confirmed latest-item comparison | E2-B defense-upgrade comparison is implemented structurally: after the latest reward is resolved, the normal `Next:` hint can recommend Wall, Tower, or Defenders from existing affordable upgrade data, or show the missing Gold/Scrap for Wall when wall/pressure is the current blocker. Focused Play Mode must still verify the upgrade recommendation remains readable and leads cleanly back to Push or the next contract. |
