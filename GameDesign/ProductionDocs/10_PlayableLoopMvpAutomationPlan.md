...
**Last meaningful movement:** E2-A no longer blocks the fresh-save recovery check before a save exists. The `Load` action remains available when `DefenseSaveManager` exists, so a first-session player can click it and see the guided frontline -> contract -> reward -> save path instead of a raw missing-save path. First recovery-save copy now also says the recovery point covers HUD settings.

**Next unlock:** `E2-A | P1 | Onboarding, settings, recovery | In progress / Fresh-save recovery path unblocked`. Remaining unlock evidence is focused fresh-save/recovery Play Mode QA, plus a product decision before adding a full player-facing settings menu beyond the current HUD text-density/guide toggles.
...
| E2-A | P1 | Onboarding, settings, recovery | In progress / Fresh-save recovery path unblocked | Teach the first-session loop after E1-A makes a real decision. | Normal HUD now uses compact status plus `Next:` guidance for start frontline -> compare contracts -> run/fail/reward -> equip/salvage -> first recovery save. Save schema v6 persists HUD text density, balance-detail visibility, diagnostic text visibility, first-session guide, and first-recovery-save emphasis. The `Load` button remains clickable before the first save so the no-save recovery guidance can be validated. Focused fresh-save/recovery Play Mode QA remains. |
