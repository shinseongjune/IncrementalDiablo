...
| Phase D - Long-Horizon Systems Foundation | Current phase. The saved depth ladder is accepted, and D0-B formula-driven threat/reward/material bands are implemented with a deterministic export/check path. | Formula-driven dungeon tiers, ground scaling, item rarity/material sinks, and save migration hooks exist without hand-authored content ladders. | Complete the focused D0-B runtime comparison, then move to D0-C item registry and migration. |
...
| Last meaningful movement | 2026-06-08: D0-B now routes selected/active depth through ten-depth formula bands. Spawned enemies receive health/damage multipliers, rewards store depth-scaled level/power, salvage restores material yield from saved item level, the HUD exposes the profile, and a depth 1-100 CSV export/check path is green. |
| Next unlock | D0-B is `In Progress / Needs focused Unity Play Mode`: compare Depth 1 and Depth 2 enemy HP/damage plus the Depth 2 reward level/power. After acceptance, promote D0-C item registry and save migration to `Next`. |
| Loop coverage | Phase A debug loop, Phase B normal HUD loop, and Phase C first real game slice are complete. Phase D now has a persistent selectable depth ladder plus formula-driven risk/reward/material profiles instead of fixed depth-1 combat and loot. Runtime acceptance of the first depth transition remains. |
| Known blockers | No compile, scene-wiring, formula, export, or save-schema blocker remains. D0-B needs one focused runtime comparison because Unity batchmode is intentionally avoided; D0-C stays blocked until that contract is accepted. |
...
| D0-B | P0 | Formula-driven depth threat and reward bands | In Progress / Needs focused Unity Play Mode | `DungeonDepthBalanceModel` defines bounded ten-depth bands; spawned enemy health/damage, item level/rolled power, and salvage material yield use the active/source depth; the HUD exposes the profile; the depth 1-100 export/check is deterministic. | Compare Depth 1 versus Depth 2 enemy HP/damage and confirm the Depth 2 reward records level 2 plus scaled power. Then mark Done and promote D0-C. |
| D0-C | P0 | Item registry and save migration | Pending | Saved authored items resolve through a production item-definition registry; unknown ids produce an explicit migration/diagnostic result instead of silently depending on runtime prototype definitions. | Integrate TD-03 and TD-07 replacement with registry and migration work. |
...
- Weak point: the first formula band is implemented but has not yet been observed against the real spawned prefab and authored reward path in Play Mode.
- Required correction: run only the focused Depth 1 versus Depth 2 comparison, fix a concrete mismatch if found, then close D0-B and move to D0-C. Do not turn this into another broad loop acceptance pass.
