...
| Last meaningful movement | 2026-05-28: `InventoryOverlayPresenter` now gives a wired `Panel_InventoryOverlay` real player-facing content: item rows, selected-item details, wallet/material preview, equip selected, salvage selected, latest selection, and close-overlay controls. This moves inventory from a HUD summary/debug action into an authored overlay path while leaving panel composition to Unity. |
| Next unlock | In Unity, add `InventoryOverlayPresenter` to the existing `Gameplay` `Panel_InventoryOverlay`, assign TMP labels for header/list/selected item/materials/message, and connect Previous/Next/Latest/Equip/Salvage/Close buttons. Then wire the bottom-action-bar `OpenInventoryOverlay` / `CloseOverlay` buttons through `PlayableLoopHud` if they are not already hooked up. Play Mode validate: clear a dungeon, open inventory overlay, select the reward, equip or salvage it, close the overlay, and confirm the previous `DefenseFocus`/`DungeonFocus` returns. `Panel_CraftingOverlay` and `Panel_RewardOverlay` are already present as overlay GameObjects but still need content behavior later. Ground-combat feel validation remains a parallel scene/feel gate. |
| Loop coverage | Phase A debug loop and Phase B player HUD slice are confirmed. Phase C now has one direct-control dungeon encounter path, visible room-presentation support, `Gameplay` wiring for `PF_DungeonEnemy_Melee` through `EnemySpawner`, authored tier-1 item definitions in the reward loop, HUD diagnostics that expose prototype fallback, a verified ground-lane presentation bridge, a `Gameplay`-wired ground combat feedback presenter with runtime combat telemetry, a reusable screen-focus/overlay controller for the first dual-panel playable screen, HUD-facing overlay controls with missing-overlay guards, an inventory overlay presenter for item review/equip/salvage, and a scriptable automation check for the current scene/docs/config contract. Play Mode validation for real ground-defense feel, the authored dungeon room shell, layout transition readability, inventory overlay composition, crafting/reward overlay content, and long-term item registry/drop-table tooling remain open. |
| Known blockers | There is no current code build blocker. Product blockers remain: `GroundDefenseCombatPresenter` needs Play Mode feel validation; direct-control dungeon encounter and authored reward path still need full Play Mode feel/loop validation; room scale/layout, spawn placement, NavMesh feel, marker art/count, lane/camera framing, panel RectTransform composition, and final visual composition still require manual Unity judgment. Static scene checks now find `Panel_InventoryOverlay`, `Panel_CraftingOverlay`, and `Panel_RewardOverlay` wired into `PlayableScreenLayoutController`; the remaining overlay gap is content/button wiring, with inventory code ready and crafting/reward content presenters still future work. Final split ratio, camera feel, panel crop, overlay content density, and ornate UI density still require user/Unity Editor review. |
...
- This advances the normal inventory path beyond HUD-only latest-item actions. It does not wire the scene panel, craft/reroll behavior, reward reveal content, final item-list art, or long-term registry/drop-table tooling.
