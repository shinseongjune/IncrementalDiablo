...
**Last meaningful movement:** E2-A is accepted from the 2026-07-01 user-confirmed recovery guidance check. Fresh-save HUD guidance, schema-v6 HUD settings persistence, clickable no-save `Load` guidance, and the centralized `DefenseSaveManager.NoSaveRecoveryGuidance` path now form the accepted first-session recovery baseline.

**Next unlock:** `E2-B | P1 | Goal comparison clarity | Next / Product work`. Add concise normal-path guidance that helps a player name why a contract, item, or upgrade is better for the current goal without reopening the compact HUD or adding a full settings menu.
...
| E2-B | P1 | Goal comparison clarity | Next / Product work | Help a fresh player name the better next choice after recovery works. | Add compact, normal-path comparison guidance for one or more of: contract tradeoff, latest reward vs equipped item, or next defense upgrade. Keep it data-driven and avoid debug labels, full settings-menu scope, or a crowded always-on HUD. |
...
| Dungeon reward -> inventory/equipment/salvage | Working and accepted with authored affix output, encounter reward-depth offsets, and E2-A recovery guidance | E2-B should teach why the latest reward, selected contract, or upgrade is better for the current goal before adding new item breadth. |
