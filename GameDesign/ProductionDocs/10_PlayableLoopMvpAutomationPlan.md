...
| Phase D - Long-Horizon Systems Foundation | Current phase. The saved depth ladder is implemented and accepted, but threat and rewards still do not scale from depth. | Formula-driven dungeon tiers, ground scaling, item rarity/material sinks, and save migration hooks exist without hand-authored content ladders. | Connect formula-driven threat and reward bands before broader itemization or ground-scaling work. |
...
| Last meaningful movement | 2026-06-07: The user confirmed D0-A in Play Mode. The normal HUD can unlock one next depth after clearing the current highest, select/start that depth, avoid advancement on failure, and preserve selected/highest depth through save/load. |
| Next unlock | D0-B is `Next`: consult the D2 reference for pacing principles, define conservative formula-driven enemy threat, reward power, and material-yield bands, then add a balance export/check path. |
| Loop coverage | Phase A debug loop, Phase B normal HUD loop, and Phase C first real game slice are complete. Phase D now adds a persistent selectable dungeon-depth ladder to the accepted defense/combat/reward/crafting/save loop instead of repeating only fixed depth 1. Threat and rewards do not yet scale from the selected depth; that remains D0-B. |
| Known blockers | There is no current build, scene-wiring, or progression-state blocker. The product gap is D0-B: selected depth is saved and playable but does not yet change combat threat or reward/material value. |
...
| D0-B | P0 | Formula-driven depth threat and reward bands | Next | Enemy threat, reward power, and material yield scale from depth through documented formulas and bounded milestone bands rather than per-depth authored lists. | Consult the D2 reference for pacing ideas, define conservative formulas, and add a balance export/check path. |
...
- Weak point: selected depth does not yet change threat or rewards, so the new ladder is structurally visible but not yet a meaningful difficulty/reward choice.
- Required correction: make D0-B formulas drive enemy threat and reward/material bands. Do not replace that work with more diagnostics or presentation-only depth UI.
