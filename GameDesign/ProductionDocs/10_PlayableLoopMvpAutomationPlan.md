...
| Last meaningful movement | 2026-06-06: User confirmed the P0-D spawned-enemy path works in Play Mode. The accepted path now covers NavMesh-resolved prefab spawn, tracked combat, chase/attack, player attacks, HP/death, room clear, reward continuity, and retry. Normal `Gameplay` disables prototype combat simulation so a broken real-enemy path cannot silently resolve through calculation combat. |
| Next unlock | Use `3.1 Phase C MVP Completion Task List` as the canonical work selector. P0-E is now `Next`: inspect the reward, inventory, and crafting overlays as one normal player journey and remove the smallest duplicated action, unclear label, or text-only decision that still makes the path feel like a debug tool. |
| Loop coverage | Phase A debug loop and Phase B player HUD slice are confirmed. Phase C now has an accepted direct-control dungeon encounter using `PF_DungeonEnemy_Melee`, required prefab/click-target validation, NavMesh-resolved spawning, tracked chase/attack and HP/death resolution, authored reward continuity, retry support, and no prototype combat simulation in normal `Gameplay`. It also has authored tier-1 item definitions with first-Rare/pity access, a verified ground-lane presentation bridge, accepted ground actor behavior, defense alerts during dungeon focus, dual-panel camera/input routing, saved-running focus synchronization, overlay controls, inventory/reward/crafting presenters, reward auto-open, and a confirmed paid Rare reroll path. Reward/inventory/crafting normal-path confusion cleanup, full-loop acceptance, and long-term item registry/drop-table tooling remain open. |
| Known blockers | There is no current code build blocker. P0-A through P0-D are done. Product blockers are now P0-E reward/inventory/crafting normal-path acceptance and P0-F full-loop acceptance. The current fixed-slot ground actor projection remains replacement debt and must not receive more blockout polish. |
...
| P0-E | P0 | Reward, inventory, crafting normal path | Next / Needs focused normal-path review | Reward overlay, inventory overlay, and crafting overlay support normal player actions without debug-HUD dependence; duplicated actions and text-only confusion are reduced enough for MVP. | Run clear -> reward review -> equip or salvage -> inventory review -> crafting reroll -> close/return. Record the first duplicated action, unclear label, disabled-button confusion, or unnecessary HUD dependency and fix the smallest persistent issue. |
