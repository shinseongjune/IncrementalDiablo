...
| Last meaningful movement | 2026-06-03: Phase C now has an explicit MVP completion checklist in this document. The checklist makes camera/screen readability a P0 gate, keeps crafting/dungeon/ground-combat validation ahead of support polish, and classifies `Defense alert` as a support acceptance detail rather than a standalone MVP feature track. |
| Next unlock | Use `3.1 Phase C MVP Completion Task List` as the canonical work selector. Current `Next` is P0-A crafting validation: with no Rare in inventory, clear or claim the next authored dungeon reward and confirm it is Rare, salvage one Rare if `AlterStone` is still missing, claim the next Rare, open crafting from the bottom action bar, confirm the overlay auto-selects the rerollable Rare, reroll the Rare affix, confirm materials are spent, confirm the affix text changes rather than repeating the same prototype affix, confirm the Result panel shows the spent cost plus before/after affix summary, confirm equipped hero stats refresh if the item is equipped, salvage a spare item, close overlays, and confirm the previous focus returns correctly. P0-B camera/screen readability and P0-C ground-combat feel are parallel Play Mode gates, not optional polish. |
| Loop coverage | Phase A debug loop and Phase B player HUD slice are confirmed. Phase C now has one direct-control dungeon encounter path, visible room-presentation support, `Gameplay` wiring for `PF_DungeonEnemy_Melee` through `EnemySpawner`, authored tier-1 item definitions in the reward loop with first-Rare/pity access, HUD diagnostics that expose prototype fallback, a verified ground-lane presentation bridge, a `Gameplay`-wired ground combat feedback presenter with runtime combat telemetry, HUD-level defense alerts for wall/pressure danger during dungeon focus, a reusable screen-focus/overlay controller for the first dual-panel playable screen, HUD-facing overlay controls with missing-overlay guards, an inventory overlay presenter for item review/equip/salvage, a user-confirmed reward overlay flow that can auto-open on dungeon clear and return to the intended gameplay focus, a crafting overlay presenter plus first-pass scene layout for Rare affix reroll/salvage with automatic reroll-candidate selection, result feedback, and prototype anti-repeat reroll behavior, a scriptable automation check for the current scene/docs/config contract, and the canonical Phase C MVP completion checklist below. Play Mode validation for crafting, camera/screen readability, real ground-defense feel, the authored dungeon room shell, layout transition readability, crafting overlay interaction/readability, and long-term item registry/drop-table tooling remain open. |
| Known blockers | There is no current code build blocker. Product blockers are now tracked in `3.1 Phase C MVP Completion Task List`: crafting validation, camera/screen readability, ground-combat feel, dungeon prefab combat feel, reward/inventory/crafting normal-path validation, and the full loop acceptance test. `Defense alert` remains a support acceptance detail that needs one readability check during `DungeonFocus`, but it must not displace the P0 camera/combat/crafting gates. Static scene checks now find `Panel_InventoryOverlay`, `Panel_CraftingOverlay`, and `Panel_RewardOverlay` wired into `PlayableScreenLayoutController`, and the reward overlay auto-open path has user confirmation. Final split ratio, camera feel, overlay content density, reward reveal treatment, crafting panel density, and ornate UI density still require user/Unity Editor review. |
...
| --- | --- | --- | --- | --- | --- |
| P0-A | P0 | Crafting validation | Next / Needs Unity Play Mode | Rare acquisition, salvage for `AlterStone`, Rare affix reroll spend, changed affix text, Result before/after summary, equipped stat refresh, spare-item salvage, overlay close, and previous-focus return are all confirmed in Play Mode. | Record Play Mode evidence and mark `Done`, or list the exact blocker. |
| P0-B | P0 | Camera and screen readability | Needs Unity Play Mode / Manual Unity judgment | `DefenseFocus` camera, `DungeonFocus` camera, the 70/30 split, defense side-panel crop, overlay occlusion, and alert readability are acceptable for MVP. | Record exact camera/layout values or manual blockers. Do not bury camera under generic UI polish. |
...
| P0-D | P0 | Dungeon prefab combat feel | Needs Unity Play Mode | `PF_DungeonEnemy_Melee` spawns, activates on `Running`, chases/attacks, takes direct player click attacks, shows HP/result feedback, and clears into the reward path. | Record spawn, NavMesh, HP, click-combat, camera, and room-shell blockers. |
...
| P0-F | P0 | Full loop acceptance test | Blocked until P0-A through P0-E pass | A 10-20 minute run proves defense income, dungeon start, room combat, loot, equip or salvage, crafting reroll, reinvestment, save/load, and one failure/recovery path. | Run only after upstream gates are `Done` or consciously waived. |
