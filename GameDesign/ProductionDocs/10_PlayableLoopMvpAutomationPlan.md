...
| Last meaningful movement | 2026-06-19: User accepted E0-A3 formula-driven battle scale in Unity Play Mode. The high-band density/role mix remains readable in the defense view, with the non-save review override and HUD label available only as verification support. |
| Next unlock | E0-B should align the defense camera/composition toward the reference-image battlefield framing while preserving the accepted E0-A readable units, attacker ownership, density, and automatic no-micromanagement behavior. |
...
| Known blockers | No compile, runtime-combat, save, item, or economy blocker is known. The next open visual issue is camera/reference-image composition: the battle reads, but the camera position/framing still needs a focused Unity Editor pass to match the reference target. |
...
| E0-B | P0 | Defense camera and reference composition pass | Next / Needs Unity Editor composition judgment | Preserve the accepted E0-A actor identity, attacker ownership, role mix, density, and automatic behavior while moving the defense camera/framing toward `GameDesign/References/2026-05-17_FinalGameplayScreenConcept.png`. Do not add unit controls, manual waves, or a new presentation abstraction. | Focused manual pass: tune defense camera position/angle/orthographic size or panel crop conservatively; validate real Level 1 and E0-A3 review-band density in both `DefenseFocus` and the compressed panel. |
