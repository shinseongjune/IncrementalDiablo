...
| Last meaningful movement | 2026-06-05: User accepted the P0-C ground-defense behavior as working and explicitly rejected further investment in meaningless temporary presentation. `GroundDefenseActorRuntime` now closes the behavior-readability gate, while the fixed three-slot/blockout presentation is frozen as a replacement target rather than a surface to polish. |
| Next unlock | Use `3.1 Phase C MVP Completion Task List` as the canonical work selector. P0-D is now `Next`: validate and improve the production-persistent dungeon prefab combat path so `PF_DungeonEnemy_Melee` visibly spawns, chases, attacks, takes player attacks, shows HP/result feedback, dies, and clears into the reward path without prototype simulation. |
| Loop coverage | Phase A debug loop and Phase B player HUD slice are confirmed. Phase C now has one direct-control dungeon encounter path, visible room-presentation support, `Gameplay` wiring for `PF_DungeonEnemy_Melee` through `EnemySpawner`, authored tier-1 item definitions in the reward loop with first-Rare/pity access, HUD diagnostics that expose prototype fallback, a verified ground-lane presentation bridge, a user-accepted discrete ground actor behavior bridge with individual health/hit/defeat/contact state plus automatic attack and wall feedback, HUD-level defense alerts for wall/pressure danger during dungeon focus, a reusable screen-focus/overlay controller for the first dual-panel playable screen, scene-wired render-target and routed-input components for putting the dungeon camera into a UI panel, HUD-facing viewport diagnostics for render/input status, saved-running dungeon focus synchronization, safer stationary target attack behavior, HUD-facing overlay controls with missing-overlay guards, an inventory overlay presenter for item review/equip/salvage, a user-confirmed reward overlay flow that can auto-open on dungeon clear and return to the intended gameplay focus, a user-confirmed crafting validation path for Rare acquisition, `AlterStone` salvage, paid Rare affix reroll, before/after result proof, equipped-stat refresh, spare salvage, and overlay focus return, plus an automation-owned prototype debt register/scan path. The authored dungeon room combat feel, reward/inventory/crafting normal-path confusion cleanup, full-loop acceptance, and long-term item registry/drop-table tooling remain open. |
| Known blockers | There is no current code build blocker. P0-A crafting validation, P0-B camera/screen readability, and P0-C ground-defense behavior readability are done. Product blockers are now P0-D dungeon prefab combat feel, P0-E reward/inventory/crafting normal-path validation, and P0-F full-loop acceptance. The current fixed-slot ground actor projection is tracked in `12_PrototypeDebtRegister.md` and must not receive more color/count/speed/blockout polish; revisit it only for a regression or when replacing it with pooled prefabs, archetype data, real targeting, and production feedback. |
...
| P0-D | P0 | Dungeon prefab combat feel | Next / Needs Unity Play Mode | `PF_DungeonEnemy_Melee` spawns, activates on `Running`, chases/attacks, takes direct player click attacks, shows HP/result feedback, and clears into the reward path. | Record spawn, NavMesh, HP, routed panel click-combat, death/clear, reward continuity, and room-shell blockers; implement the smallest production-persistent fix instead of another diagnostic layer. |
...
| P0-F | P0 | Full loop acceptance test | Blocked until P0-D and P0-E pass | A 10-20 minute run proves defense income, dungeon start, room combat, loot, equip or salvage, crafting reroll, reinvestment, save/load, and one failure/recovery path. | Run only after upstream gates are `Done` or consciously waived. |
...
- Required correction: the P0-C ground behavior bridge is sufficient for the current gate. Phase C work must now default to production-persistent dungeon combat and normal-path UX, not further temporary ground actor presentation.
...
- Operating rule: unless a regression or build blocker appears, do not spend another run polishing the current defense blockout. The next meaningful production increment should make the authored dungeon prefab combat path more complete and player-facing.
