...
- Do not choose debt cleanup ahead of Phase C P0 gates unless the cleanup directly unblocks camera/readability, combat feel, normal reward/inventory/crafting flow, or build verification.
- After P0-F passes, select the first safe `Alpha blocker` debt item before opening broad Phase D feature work.
...
| Last meaningful movement | 2026-06-04: Fixed two P0-B Play Mode defects after the camera/input bridge and prototype-debt automation. A saved `Running` dungeon now drives screen focus back to `DungeonFocus` instead of spawning enemies behind the defense screen, and Shift-click stationary target attacks now keep waiting for range instead of clearing after one out-of-range swing. |
| Next unlock | Use `3.1 Phase C MVP Completion Task List` as the canonical work selector. Current P0-B is code-ready but needs Unity Editor wiring and Play Mode judgment: add a `RawImage` under `Panel_DungeonViewport`, bind a dungeon camera through `PanelCameraRenderTarget`, wire `DungeonViewportInputRouter`, then validate saved-running dungeon focus restore, `DefenseFocus` camera/readability, `DungeonFocus` camera/readability, the MVP 70/30 split, defense side-panel crop, overlay occlusion, routed click movement/attack, Shift-click target hit behavior, and defense-alert readability. |
| Loop coverage | Phase A debug loop and Phase B player HUD slice are confirmed. Phase C now has one direct-control dungeon encounter path, visible room-presentation support, `Gameplay` wiring for `PF_DungeonEnemy_Melee` through `EnemySpawner`, authored tier-1 item definitions in the reward loop with first-Rare/pity access, HUD diagnostics that expose prototype fallback, a verified ground-lane presentation bridge, a `Gameplay`-wired ground combat feedback presenter with runtime combat telemetry, HUD-level defense alerts for wall/pressure danger during dungeon focus, a reusable screen-focus/overlay controller for the first dual-panel playable screen, render-target and routed-input components for putting the dungeon camera into a UI panel, saved-running dungeon focus synchronization, safer stationary target attack behavior, HUD-facing overlay controls with missing-overlay guards, an inventory overlay presenter for item review/equip/salvage, a user-confirmed reward overlay flow that can auto-open on dungeon clear and return to the intended gameplay focus, a user-confirmed crafting validation path for Rare acquisition, `AlterStone` salvage, paid Rare affix reroll, before/after result proof, equipped-stat refresh, spare salvage, and overlay focus return, plus an automation-owned prototype debt register/scan path. Play Mode validation for camera/screen readability, RawImage routed click combat, real ground-defense feel, the authored dungeon room shell, layout transition readability beyond the P0-A path, reward/inventory/crafting normal-path confusion cleanup, and long-term item registry/drop-table tooling remain open. |
| Known blockers | There is no current code build blocker. Product blockers are now tracked in `3.1 Phase C MVP Completion Task List`: camera/screen readability, ground-combat feel, dungeon prefab combat feel, reward/inventory/crafting normal-path validation, and the full loop acceptance test. P0-A crafting validation is done. P0-B has code support but still requires Unity Editor scene wiring for the dungeon `RawImage`, dungeon camera, render target, and input router, then Play Mode readability/click validation including save/restart while a dungeon is running. `Defense alert` remains a support acceptance detail that needs one readability check during `DungeonFocus`, but it must not displace the P0 camera/combat gates. Prototype/debug/fallback cleanup is now tracked in `12_PrototypeDebtRegister.md`; alpha-blocking rows must be addressed after P0-F or when they directly block a P0 gate. Static scene checks now find `Panel_InventoryOverlay`, `Panel_CraftingOverlay`, and `Panel_RewardOverlay` wired into `PlayableScreenLayoutController`, and the reward overlay auto-open path plus P0-A crafting path have user confirmation. Final split ratio, camera feel, overlay content density, reward reveal treatment, crafting panel density, and ornate UI density still require user/Unity Editor review. |
...
| P0-B | P0 | Camera and screen readability | In Progress / Code bridge ready / Needs Unity wiring and Play Mode | `DefenseFocus` camera, `DungeonFocus` camera, the 70/30 split, defense side-panel crop, overlay occlusion, saved-running dungeon focus restore, routed dungeon-panel click movement/attack, Shift-click target hits, and alert readability are acceptable for MVP. | Wire `PanelCameraRenderTarget` plus `DungeonViewportInputRouter` in `Gameplay`, then record exact camera/layout values or manual blockers. Do not bury camera under generic UI polish. |
| P0-C | P0 | Ground defense readable combat | In Progress / Needs Unity Play Mode | Pressure actors, wall contact, defender/tower attacks, wall damage, and frontline pressure read as visible automatic combat rather than only abstract counters. | Validate `GroundDefenseCombatPresenter` in Play Mode; if the bridge is insufficient, implement the smallest real defense actor pass. |
| P0-D | P0 | Dungeon prefab combat feel | Needs Unity Play Mode | `PF_DungeonEnemy_Melee` spawns, activates on `Running`, chases/attacks, takes direct player click attacks, shows HP/result feedback, and clears into the reward path. | After P0-B wiring, record spawn, NavMesh, HP, routed panel click-combat, camera, and room-shell blockers. |
| P0-E | P0 | Reward, inventory, crafting normal path | In Progress | Reward overlay, inventory overlay, and crafting overlay support normal player actions without debug-HUD dependence; duplicated actions and text-only confusion are reduced enough for MVP. | Validate reward/crafting flow and list any confusing duplicate actions or unclear labels. |
| P0-F | P0 | Full loop acceptance test | Blocked until P0-B through P0-E pass | A 10-20 minute run proves defense income, dungeon start, room combat, loot, equip or salvage, crafting reroll, reinvestment, save/load, and one failure/recovery path. | Run only after upstream gates are `Done` or consciously waived. |
| P1-A | P1 | MVP presentation polish | Pending | Top/bottom bars, buttons, labels, overlay density, text fit, and non-debug visual hierarchy are acceptable for the first sellable-slice direction. | Start after P0 camera/combat/crafting gates are accepted. |
| P1-B | P1 | Authored item minimum breadth | Code Done / Needs Unity Play Mode | Existing tier-1 authored items create readable Normal/Magic/Rare choices without expanding content prematurely. | Confirm through reward, equip, salvage, and reroll validation. |
| P1-C | P1 | Prototype/debug/fallback debt retirement | Automation Ready / Deferred until P0-F unless it unblocks a P0 gate | Registered prototype/debug/fallback paths each have a keep/promote/replace/delete decision, retirement trigger, and alpha-blocker flag. | Run `Get-PrototypeDebtInventory.ps1` every automation run, update `12_PrototypeDebtRegister.md` when markers change, and after P0-F select the first safe alpha-blocking cleanup. |
| SUP-01 | Support | Defense alert readability | Code Done / Needs Unity Play Mode | During `DungeonFocus`, wall/pressure/breach danger can be noticed through the HUD summary/action hint without becoming a new polish track. | Validate once while P0-B or P0-C is tested; do not select more alert polish before P0 gates. |

Deferred until after Phase C MVP: Legendary/Unique/Set item breadth, large affix pools, broad drop-rate economy balancing, auto-salvage/filter rules, multiple dungeon themes, long-tail progression tuning, Steam/platform work, cloud/network features, and release marketing surfaces.
...
## 3.2 Progress Assessment Against Game-Form Plan
