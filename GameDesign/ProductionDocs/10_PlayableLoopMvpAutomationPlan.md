...
| Last meaningful movement | 2026-06-02: `ItemInstance.TryApplyPrototypeAffixReroll(...)` now avoids repeating the currently saved prototype affix when another candidate exists for the item's slot. This keeps the first Rare reroll validation path from spending materials on a visually unchanged result while staying inside the current two-candidate prototype pool. |
