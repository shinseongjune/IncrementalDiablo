...
- players can name why one item, contract, or upgrade is better for their current goal. E2-B contract comparison was user-accepted on 2026-07-03, latest reward item comparison was user-confirmed on 2026-07-05, and defense-upgrade comparison with Wall shortfall plus post-upgrade return guidance was user-confirmed on 2026-07-08;
...
2026-07-09 status: the reproducible first-session QA checklist now exists in `06_UnitySceneAndPrefabSetupGuide.md`. It covers fresh save, no-save Load guidance, contract choice, dungeon run, reward handling, defense upgrade return, HUD settings restore, and save/load. R3 still needs a product decision on whether the next player-facing move is a small settings menu, checklist-led build handoff, or a manual boss/room visual-authoring pass.
...
| E3-A | Needs decision / P1 | Settings menu scope and first-session QA checklist for R3 readiness. | 2026-07-09: the R3 first-session QA checklist was drafted. Recommended next decision: expose only the existing persisted HUD text-density/guide toggles first. Alternatives are to use the checklist as the build handoff first, or start a manual visual-authoring pass for boss/room presentation after the user approves visual scope. |
