...
| E3-A | Needs decision / P1 | Settings menu scope and first-session QA checklist for R3 readiness. | Recommended next decision: expose only the existing persisted HUD text-density/guide toggles first. Alternatives are a reproducible fresh-save QA/build handoff, or a manual visual-authoring pass for boss/room presentation after the user approves visual scope. |
