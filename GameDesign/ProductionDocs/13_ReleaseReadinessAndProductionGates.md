...
`E1-A` is accepted from the focused Play Mode path as of 2026-06-25. `E1-B` is accepted from the focused Play Mode path as of 2026-06-26. `E1-C` is accepted from the focused Play Mode path as of 2026-06-27, so the repeatable slice now has contract choice, authored item-affix outcome, and reusable elite/boss encounter variety. `E2-A` is accepted from the 2026-07-01 user-confirmed recovery guidance check: fresh-save guidance, first recovery-save copy, schema-v6 settings persistence for current HUD text-density/guide choices, and clickable no-save `Load` guidance share the same `DefenseSaveManager` recovery copy.
...
- players can name why one item, contract, or upgrade is better for their current goal. E2-B contract comparison was user-accepted on 2026-07-03, latest reward item comparison was user-confirmed on 2026-07-05, and the current E2-B unlock is defense-upgrade comparison using the existing Wall/Tower/Defenders costs, effect deltas, and Wall shortfall guidance when wall/pressure is the blocker;
...
| E2-B | In progress / P1 | Goal comparison clarity after the first-session path is recoverable. | Contract comparison is accepted from the 2026-07-03 user-confirmed check. Latest item comparison is accepted from the 2026-07-05 user-confirmed check: the normal HUD compares the newest resolved reward item against the equipped same-slot item and keeps unresolved or unequipped latest rewards ahead of next-contract guidance. The current slice compares defense upgrades: stressed wall/pressure recommends Wall, and if Wall is not affordable the HUD shows the exact missing Gold/Scrap instead of drifting to a weaker offense goal; otherwise affordable Tower/Defenders are compared by existing DPS gain before returning to Push or the next contract. Remaining evidence is focused `Gameplay` Play Mode validation of stressed-wall shortfall text, enough Gold/Scrap -> defense upgrade `Next:` recommendation -> buy named upgrade -> save/load. |
