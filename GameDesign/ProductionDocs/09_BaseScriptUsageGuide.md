...
| `GroundDefenseActorRuntime` | `Assets/02.Scripts/GroundDefense/Runtime/GroundDefenseActorRuntime.cs` | 연속 전선 수치를 아키타입 기반 개별 압박 적 상태의 체력, 이동, 피격, 처치, 벽 접촉으로 변환하고 spawn/hit/defeat/wall-contact event를 표현 계층에 제공한다. | `Gameplay > DefenseRoot`에 붙이고 Grunt/Shield/Runner 아키타입을 연결한다. 현재 capacity는 8이다. actor state는 저장하지 않는다. | `DefenseRuntimeState`가 계속 권위 상태인지, 역할별 차이와 event presentation이 보상/저장을 중복 소유하지 않는지 |
