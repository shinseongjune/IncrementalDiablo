...
2026-06-06 automation-plan freshness scope: `Invoke-IncrementalDiabloChecks.ps1` requires the canonical plan to name `Phase D - Long-Horizon Systems Foundation` and its current production routing. After D0-A acceptance, the token contract covers completed D0-A plus next-task D0-B so the harness catches stale automation routing instead of preserving it. A future phase or task promotion must update this contract in the same run.
