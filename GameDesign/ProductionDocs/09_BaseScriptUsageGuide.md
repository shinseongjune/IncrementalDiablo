...
| `CraftingOverlayPresenter` | `Assets/02.Scripts/UI/CraftingOverlayPresenter.cs` | Fills an authored crafting overlay with item rows, selected item details, current affixes, wallet/materials, salvage preview, Rare reroll cost, reroll selected, salvage selected, and close-overlay behavior. | Current `Gameplay` already attaches it to `Panel_CraftingOverlay` and wires first-pass TMP labels/buttons. When rebuilding, attach it to `Panel_CraftingOverlay` or a child controller, wire TMP labels for header/list/selected item/materials/result/message and buttons for Previous/Next/Latest/Reroll Affix/Salvage/Close, and keep `Auto Find References` on for the first pass or explicitly assign `SimpleInventory`, `ItemSalvageService`, `EquipmentSlots`, `CurrencyWallet`, and `PlayableScreenLayoutController`. | Confirm a Rare item shows a reroll cost, reroll spends `Gold + Essence + AlterStone`, the affix list changes, equipped hero stats refresh if the item is equipped, salvage still pays materials, and close returns to the previous focus. |
