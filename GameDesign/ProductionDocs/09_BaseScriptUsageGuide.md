...
- Replace unattached wall flash feedback with wall-bound impact/damage/breach presentation.
- The prepared sheet remains a silhouette/reference source. It does not require runtime quad cards to be the final render technique.
- Keep `DefenseRuntimeState` authoritative. Do not create a second reward/progression simulation, authored wave ladder, or RTS command model.
- Use the current top section of `06_UnitySceneAndPrefabSetupGuide.md` for the next implementation handoff.
...
- The fixed `GroundDefenseActorRuntime` scene slots were replaced by the E0-A pooled prefab/archetype path. TD-08 remains open only for focused runtime acceptance.
...
2026-06-12 automation-plan freshness scope: the harness requires E0-A to remain `RTS-readable automatic defense battlefield | Next` and checks the real-projectile/reinforcement direction tokens. Existing pooled archetype/runtime checks protect reusable foundations only; they do not count as acceptance of the billboard/pulse bridge.
...
2026-06-09 D0-C item registry scope: assign `ItemDefinitionRegistry.asset` to `SimpleInventory.definitionRegistry`. Add authored definitions there before reward use, and add an `ItemDefinitionIdMigration` entry before renaming/removing a shipped id. `DefenseSaveManager` writes schema v3, runs registry migration before validation, and exposes `LastLoadReport`. Unknown ids stay preserved but unresolved; the HUD and overlays label them, and equip/salvage/reroll actions remain disabled.
...
| `GroundDefenseBattlefieldView` | `Assets/02.Scripts/GroundDefense/UI/GroundDefenseBattlefieldView.cs` | 성벽, 전선 수비병, 석궁탑을 lane anchor 기준으로 생성한다. | `DefenseRoot`에서 방어 카메라, `WallAnchor`, `AttackOrigin`, readability sheet를 연결한다. 크기/offset은 겹침이 있을 때만 조정한다. | 방어선 관계가 즉시 읽히고 패널 crop을 침범하지 않는지 |
| `GroundDefenseCombatPresenter` | `Assets/02.Scripts/GroundDefense/UI/GroundDefenseCombatPresenter.cs` | 현재 runtime-to-presentation bridge다. Pulse/bolt/flash와 normal-player diagnostics는 폐기 대상이며, actor selection/telemetry helper만 필요 시 재사용한다. | 다음 E0-A 구현에서는 visible attacker-owned melee/projectile, death/reinforcement, wall-bound damage 컴포넌트로 책임을 분리한다. | 추상 pulse 없이 공격자-공격-대상 관계가 보이는지 |
| `GroundDefenseActorRuntime` | `Assets/02.Scripts/GroundDefense/Runtime/GroundDefenseActorRuntime.cs` | 연속 전선 수치를 아키타입 기반 개별 압박 적 상태의 체력, 이동, 피격, 처치, 벽 접촉 이벤트로 변환한다. | `Gameplay > DefenseRoot`에 붙이고 Grunt/Shield/Runner 아키타입을 연결한다. 현재 capacity는 8이다. | `DefenseRuntimeState`가 계속 권위 상태인지, 역할별 내구/속도 차이가 보이면서 actor state가 저장/보상을 중복 소유하지 않는지 |
