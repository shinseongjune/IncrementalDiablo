...
| TD-06 | MVP temporary screen/camera/layout values | Temporary MVP values | `GameDesign/ProductionDocs/11_PlayableScreenPresentationSpec.md`, `Assets/02.Scripts/UI/PlayableScreenLayoutController.cs`, `Assets/02.Scripts/UI/PanelCameraRenderTarget.cs` | Temporary values unblock P0-B camera/readability validation before final art direction. | P0-B records accepted camera/layout values or exact manual blockers. | Promote accepted values into first-slice defaults, or split a Unity Editor tuning task. | No, unless values remain unreviewed after P0-F. | 2026-06-04: Registered for automation sweep. |
