...
- When the override is active, the normal HUD summary adds an `E0-A3 review scale` line so the reviewer can see the visual level, band, force count, role tier, and bounded respawn cadence being shown.
- The line is intentionally hidden when the override is off. It is review status, not final player HUD copy.
- User Play Mode feedback accepted the higher-band readability on 2026-06-19. The remaining screen-presentation issue is camera/reference-image composition while preserving faction identity, attacker-to-target ownership lines, target reaction, death/reinforcement, and wall-damage readability in both `DefenseFocus` and the compressed panel.
...
- E0-A3 was accepted on 2026-06-19. For regressions or camera/composition work, review the default baseline first, then a higher Frontline band that shows added count/roles/cadence.
...
- E0-A3 formula-driven density was accepted on 2026-06-19. Later camera/composition work must not hide the accepted ownership read at actual panel scale.
